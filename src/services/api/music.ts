...
// Deezer public API — no auth, 30s previews
// TODO: replace with Jamendo (full songs) once client_id registered at developer.jamendo.com
...
export const musicApi = {
  ...
  search: async (q: string, _genre?: string, page = 0, limit = 20): Promise<Track[]> => {
    const { data } = await deezer.get('/search', { params: { q, limit, index: page * limit } });
    return (data.data ?? []).map(toTrack).filter((t: Track) => t.stream_url);
  },
